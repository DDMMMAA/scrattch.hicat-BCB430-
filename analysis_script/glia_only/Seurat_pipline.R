...
# 5. Save the result
ggsave("data/Result/cluster/non-neuro/Cell_Demographics_PieCharts.png", 
...
saveRDS(global_de_results, file = "data/Result/cluster/non-neuro/global_DE_results.rds")
...
p <- ggplot(plot_data, aes(x = cluster, y = Count, fill = cluster)) +
  ...
  labs(
    ...
    subtitle = "Whole Non-neuronal DE result",
    x = "Group",
...
# 5. Save and Print
ggsave("data/Result/cluster/non-neuro/Global_DE_Counts.png", p, width = 10, height = 6)
print(p)
...
saveRDS(de_results, file = "data/Result/cluster/non-neuro/DE_results.rds")
...
# 4. Display and Save
print(p)
ggsave("data/Result/cluster/non-neuro/DE_Counts_Summary.png", p, width = 10, height = 8)
...
# Loop through the nested list to get raw DE counts
for (subclass in names(de_results)) {
  for (variable in names(de_results[[subclass]])) {
...
avg_genes_df <- glia_only_seurat@meta.data %>%
  group_by(subclass_label) %>%
  summarise(Avg_NonZero_Genes = mean(nFeature_RNA, na.rm = TRUE))

# Merge average metric into plot_data
plot_data <- merge(plot_data, avg_genes_df, by.x = "Subclass", by.y = "subclass_label")

# B. Union Metric: Extract the sparse count matrix (Seurat v5 standard)
count_matrix <- LayerData(glia_only_seurat, assay = "RNA", layer = "counts")

union_genes_list <- list()

for (subclass in unique(plot_data$Subclass)) {
  
  # Find the cell barcodes that belong to this subclass
  subclass_cells <- rownames(glia_only_seurat@meta.data[glia_only_seurat@meta.data$subclass_label == subclass, ])
  
  # Subset the expression matrix to only include these cells
  sub_matrix <- count_matrix[, subclass_cells, drop = FALSE]
  
  # Calculate the union: Count how many genes have total expression > 0 across these cells
  union_count <- sum(Matrix::rowSums(sub_matrix) > 0)
  
  union_genes_list[[length(union_genes_list) + 1]] <- data.frame(
    Subclass = subclass,
    Union_NonZero_Genes = union_count
  )
}

union_genes_df <- do.call(rbind, union_genes_list)
...
# Union Normalized: Percentage (%) of subclass's total expressed genes
...
p_raw <- ggplot(plot_data, aes(x = Subclass, y = Count, fill = Variable)) +
  geom_bar(stat = "identity", position = "dodge") + 
  coord_flip() + 
  labs(
    title = "A. Raw DE Genes",
  ...
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), hjust = -0.1, size = 3)

# Plot B: Average Normalized (Updated to %)
p_avg <- ggplot(plot_data, aes(x = Subclass, y = Avg_Normalized_Count, fill = Variable)) +
  geom_bar(stat = "identity", position = "dodge") + 
  coord_flip() + 
  labs(
    title = "B. Average Normalized",
    subtitle = "% of avg detected genes/cell",
    x = "", 
    y = "% of Avg Transcriptome DE",
    fill = "Criteria"
  ) +
  theme_bw() +
  theme(
  ...
  geom_text(aes(label = round(Avg_Normalized_Count, 2)), position = position_dodge(width = 0.9), hjust = -0.1, size = 3)

# Plot C: Union Normalized
p_union <- ggplot(plot_data, aes(x = Subclass, y = Union_Normalized_Count, fill = Variable)) +
  geom_bar(stat = "identity", position = "dodge") + 
  coord_flip() + 
  labs(
    title = "C. Union Normalized",
    subtitle = "% of subclass expressed transcriptome",
    x = "", 
    y = "% of Total Transcriptome DE",
    fill = "Criteria"
  ) +
  theme_bw() +
  theme(
...
# Save to disk
ggsave("data/Result/cluster/non-neuro/DE_Counts_3Way_Comparison.png", final_comparison_plot, width = 20, height = 8)

# Save the final consolidated table
write.csv(plot_data, "data/Result/cluster/non-neuro/DE_Counts_All_Metrics_Summary.csv", row.names = FALSE)
message("Three-panel plot and summary data successfully saved!")
...
saveRDS(subclass_go_results, file = "data/Result/cluster/non-neuro/subclass_GO_results.rds")
